...
import { TranscriptionSegment } from "@/hooks/useCombinedTranscriptions";

export default function TranscriptionView() {
  const { combinedTranscriptions, addSystemMessage } = useCombinedTranscriptions();
  ...
  // Listen for custom events to add system messages
  React.useEffect(() => {
    const handleAddSystemMessage = (event: CustomEvent<{ message: string }>) => {
      if (event.detail && event.detail.message) {
        addSystemMessage(event.detail.message);
      }
    };
    
    document.addEventListener('add-system-message', handleAddSystemMessage as EventListener);
    
    return () => {
      document.removeEventListener('add-system-message', handleAddSystemMessage as EventListener);
    };
  }, [addSystemMessage]);
          ...
          <div
            ...
            className={
              segment.role === "assistant"
                ...
                : segment.role === "system"
                ? "p-2 w-full text-center italic text-gray-400"
